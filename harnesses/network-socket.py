...
# Make connection to target and save as pwntubes object in io
try: io = remote(TARGET_HOST,TARGET_PORT)
except socket.gaierror as e:                  print(f"{ANSI.FAIL}Socket Error: Name resolution failure ({type(e)}){ANSI.ENDC}"); exit()
except pwnlib.exception.PwnlibException as e: print(f"{ANSI.FAIL}Socket Error: Could not connect to target ({type(e)}){ANSI.ENDC}"); exit()
except Exception as e:                        print(f"{ANSI.WARNING}Socket Error: Unknown error occured ({type(e)}){ANSI.ENDC}")
